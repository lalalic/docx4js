...
export default class ZipDocument {
  ...
  getDataPartAsUrl(name, type = "*/*") {
    ...
    if (!this._shouldReleased.has(crc32)) {
      this._shouldReleased.set(
        crc32,
        URL.createObjectURL(new Blob([this.getDataPart(name)], { type }))
      );
    }
  ...
  getObjectPart(name) {
    ...
    if (!part) return null;
    else if (part.cheerio) return part;
    else {
      const $ = Object.assign(
        (this.parts[name] = this.constructor.parseXml(part.asText())),
        { part: name }
      );
...
class ContentDomHandler extends DomHandler {
  _addDomElement(el) {
    if (
      el.type == "text" &&
      (el.data[0] == "\r" || el.data[0] == "\n") //remove format whitespaces
    );
    else return super._addDomElement(el);
  }
}
