...
export default class extends Part {
  _init() {
    ...
    const transform = (ph) => ({
      ...
      tidy_schemeClr: ({ val, ...effect }) => {
        return this.doc.asColor(
          val == "phClr" ? ph.color : this.theme.color(val),
          effect
        );
      },
    });
    ...
    Object.assign(this.theme, {
      font(typeface) {
        ...
        return this(
          `a\\:fontScheme>a\\:${type[a]}Font>a\\:${b == "lt" ? "latin" : b}`
        ).attr("typeface");
      },
      color(k) {
  ...
  static identify(wXml, officeDocument) {
    ...
    if (identities[tag]) return identities[tag](...arguments);
