...
export default class Part {
  ...
  getRelObject(target) {
    const normalizedtarget = target.replace(this.folder, "");
  ...
  getRel(id) {
    ...
    switch (rel.attr("Type").split("/").pop()) {
      case "image":
        const path = target.indexOf("/") === 0 ? target : this.folder + target;
        let url = this.doc.getDataPartAsUrl(path, "image/*");
        let crc32 = this.doc.getPartCrc32(path);
        return { url, crc32 };
      default:
        if (target.endsWith(".xml")) return this.getRelObject(target);
        else return this.doc.getPart(this.folder + target);
    }
  }
  ...
  _nextrId() {
    return (
      Math.max(
        ...this.rels("Relationship")
          .toArray()
          .map((a) => parseInt(a.attribs.Id.substring(3)))
      ) + 1
    );
  }
  ...
  add(type, target, data) {
    ...
    this.rels("Relationships").append(
  ...
  addImage(data, { ext, mime } = { ext: "jpg", mime: "image/jpg" }) {
    ...
    let targetName =
      ...
      (Math.max(
        0,
        ...this.rels("Relationship[Type$='image']")
          .toArray()
          .map((t) => {
  ...
  addExternalImage(url) {
    ...
    this.rels("Relationships").append(
      `<Relationship Id="${id}" Type="${type}" TargetMode="External" Target="${url}"/>`
    );
  ...
  addChunk(data, relationshipType, contentType, ext) {
    ...
    this.rels("Relationships").append(
      `<Relationship Id="${rId}" Type="${relationshipType}" Target="${targetName}"/>`
    );

    this.doc.contentTypes.append(
  ...
  renderNode(
    ...
    identify = (node) => node.name.split(":").pop(),
    extra
  ) {
    ...
    if (Array.isArray(children)) {
      if (children.length) {
        childElements = children
          .map((a) => (a ? this.renderNode(a, createElement, identify) : null))
          .filter((a) => !!a);
      }
    }
