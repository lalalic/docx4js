...
export default class extends Base {
  ...
  render(
    createElement,
    identify = this.constructor.identify.bind(this.constructor)
  ) {
    let styles, numbering;
    if (this.styles)
      styles = this.renderNode(
        this.styles("w\\:styles").get(0),
        createElement,
        identify
      );
    if (this.numbering)
      numbering = this.renderNode(
        this.numbering("w\\:numbering").get(0),
        createElement,
        identify
      );

    return this.renderNode(
      this.content("w\\:document").get(0),
      createElement,
      identify,
      { styles, numbering }
    );
  ...
  parse(
    ...
    if (this.styles)
      doc.styles = this.renderNode(
        this.styles("w\\:styles").get(0),
        createElement,
        _identify
      );
    if (this.numbering)
      doc.numbering = this.renderNode(
        this.numbering("w\\:numbering").get(0),
        createElement,
        _identify
      );
    doc.document = this.renderNode(
      this.content("w\\:document").get(0),
      createElement,
      _identify
    );
  ...
  static identities = {
    ...
    sectPr(wXml, officeDocument) {
      const hf = (type) =>
        wXml.children
          .filter((a) => a.name == `w:${type}Reference`)
          .reduce((headers, a) => {
            headers.set(
              a.attribs["w:type"],
              officeDocument.getRel(a.attribs["r:id"])
            );
            return headers;
          }, new Map());
    ...
    p(wXml, officeDocument) {
      ...
      let identity = {
        type,
        pr: wXml.children.find(({ name }) => name == "w:pPr"),
        children: wXml.children.filter(({ name }) => name != "w:pPr"),
      };
    ...
    r(wXml) {
      return {
        type: "r",
        pr: wXml.children.find(({ name }) => name == "w:rPr"),
        children: wXml.children.filter(({ name }) => name != "w:rPr"),
      };
    },
    fldChar(wXml) {
    ...
    pic(wXml, officeDocument) {
      ...
      const props = $.props({
        ...
        tidy: ({
          spPr,
          nvPicPr: { cNvPr = {}, cNvSpPr = {}, nvPr = {} },
          style: { lnRef = {}, fillRef = {}, effectRef = {} } = {},
          ...others
        }) => ({
          ...lnRef,
          ...fillRef,
    ...
    wsp(wXml, officeDocument) {
      ...
      const props = $.props({
        ...drawml(officeDocument),
        ...same(
          "r,t,l,b".split(",").map((a) => `${a}Ins`),
          (v) => officeDocument.doc.emu2Px(v)
        ),
        filter: `:not(${content})`,
        tidy: ({
          cNvSpPr = {},
    ...
    sdt(wXml, officeDocument) {
      ...
      if (elBinding) {
        ...
        let type = "text,picture,docPartList,docPartObj,comboBox,dropDownList,date,checkbox,repeatingSection,repeatingSectionItem"
          .split(",")
          .find((a) => a == name);
        let model = { children };
        if (type) {
        ...
        switch (model.type) {
          ...
          case "control.comboBox": {
            ...
            model.options = $(elType)
              ...
              .get();
            model.value = (
              model.options.find((a) => a.displayText == selected) || {}
            ).value;
            break;
          }
          case "control.checkbox": {
            ...
            model.checked =
              $(elType).find(`${ns}\\:checked`).attr(`${ns}:val`) == "1";
            break;
          }
          case "control.text":
      ...
      return wXml.children.reduce(
        (state, node) => {
          switch (node.name) {
            case "w:trPr":
              state.pr = node;
              state.isHeader = !!node.children.find(
                (a) => a.name == "w:tblHeader"
              );
              break;
            default:
              state.children.push(node);
    ...
    altChunk(wXml, officeDocument) {
      ...
      let partName =
        ...
        officeDocument.rels(`[Id=${rId}]`).attr("Target");
      let contentType = officeDocument.doc
        .contentTypes(`Override[PartName='${partName}']`)
        .attr("ContentType");
    ...
    num(wXml) {
      return {
        ...
        abstractNum: wXml.children.find((a) => a.name == "w:abstractNumId")
          .attribs["w:val"],
      };
    },
    latentStyles() {
