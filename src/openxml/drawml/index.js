...
export default (od) => ({
  filter: ":not(a\\:extLst)",
  id: () => undefined,
  ...same("latin,ea,cs".split(","), ({ attribs: { typeface = "" } }) =>
    od.theme.font(typeface)
  ),

  ...same(
    "lumMod,lumOff,tint,shade".split(","),
    ({ attribs: { val } }) => parseInt(val) / 100000
  ),
  tidy_schemeClr: ({ val, ...effect }) =>
    od.doc.asColor(od.theme.color(val), effect),
  ...
  blip: (n) => {
    const {
      attribs: { "r:embed": embed, "r:link": url },
    } = n;
    if (url) return { url };
          ...
          .forEach((a) => {
            switch (a.name.split(":").pop()) {
              case "moveTo":
                d.push(
                  "M " +
                    px(a.children[0].attribs.x) +
                    " " +
                    px(a.children[0].attribs.y)
                );
                break;
              case "lnTo":
                d.push(
              ...
              case "cubicBezTo":
                ...
                d.push(
                  "Q " +
                    px(a.children[1].attribs.x) +
                    " " +
                    px(a.children[1].attribs.y) +
                    " " +
                    px(a.children[2].attribs.x) +
                    " " +
                    px(a.children[2].attribs.y)
                );
                break;
              case "arcTo":
                d.push(`A`);
  ...
  ...same(
    "ln,lnB,lnR,lnL,lnT,lnTlToBr,lnBlToTr".split(",").map((a) => "tidy_" + a),
    ({ w, ...props }) => ({ ...props, w: od.doc.emu2Px(w) })
  ),
  ...same(
    "left,right,top,bottom".split(",").map((a) => "tidy_" + a),
    ({ ln }) => ln
  ),
