...
describe("loader", function () {
  ...
  it("parsed without line-feed text node", function () {
    ...
    expect(doc.getObjectPart("content").root().length).toBe(
      doc.getObjectPart("trimed").root().length
    );
  ...
  it("can be cloned", function () {
    return openxml.load(`${loader}.docx`).then((docx) => {
      ...
      expect(docx.officeDocument.content == cloned.officeDocument.content).toBe(
        false
      );
      expect(docx.officeDocument.content.xml()).toBe(
        cloned.officeDocument.content.xml()
      );
    });
  });
  ...
  describe("openxml", function () {
    ...
    it("can load document with absolute urls", function () {
      return docx4js
        .load(`${__dirname}/files/DocumentWithAbsoluteUrls.docx`)
        .then(check);
    });
